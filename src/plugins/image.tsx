...
import { isImageSrcSafe } from "../utils/sanitize";
    ...
    return {
        ...
        renderButton: (props: ButtonProps & { editorAPI?: EditorAPI }) => {
            ...
            const handleInsertImage = () => {
                ...
                // but block data:image/svg via isImageSrcSafe).
                const srcWithoutMeta = src.split("|__aid__:")[0];
                if (!isImageSrcSafe(srcWithoutMeta)) {
        ...
        // Intentionally no `execute`: the image flow lives entirely inside
        // renderButton (modal state must be owned by React, not the EditorAPI).
        // The toolbar falls back to onClick on the rendered button, which opens
        // the modal. Programmatic insertion goes through editor.executeCommand(
        // "insertImage", url) instead.
        canExecute: () => true,
    };
}
